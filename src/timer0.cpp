...
timerZero *timerZero::SINGLETON {};

timerZero *timerZero::getInstance(void) {
    return SINGLETON;
}

// Default constructor
timerZero::timerZero(void) {
    this->count = 0; 
...
void timerZero::setPeriod(unsigned long int period) {
    ...
    for (int i = 0; i < 5 && !timerSet; i++) {
        if (cycles < TIMER1_MAX_VALUE * prescalerValues[i]) {
            ...
            OCR0A = cycles / prescalerValues[i]; // Setting the value with which the counter (TCNT1) will be compared
            sei(); // Re-enabling interrupts
            timerSet = true;
        }
    }
...
void timerZero::attachInterrupt(void (*isr)(void)) {
    this->_isrCallback = isr; // Setting the function to be called on the interrupt capture
    TIMSK0 |= (1 << OCIE0A);; // Enabling interrupts capture
}

void timerZero::detachInterrupt(void) {
    TIMSK0 = 0; // Disabling interrupts capture
...
void (*timerZero::getCallback(void))(void) {
...
timerZero timerZero = *timerZero::getInstance();
